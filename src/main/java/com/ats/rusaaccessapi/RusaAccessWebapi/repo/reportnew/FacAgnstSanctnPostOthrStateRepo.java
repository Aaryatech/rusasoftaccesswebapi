...
public interface FacAgnstSanctnPostOthrStateRepo extends JpaRepository<FacAgnstSanctnPostOthrState, Integer> {
	...
	@Query(value="SELECT UUID() as uniq_id, m_institute.institute_name,m_academic_year.year_id,m_academic_year.academic_year,\n" + 
			...
			"FROM m_institute,m_academic_year WHERE m_institute.institute_id=:instId and m_academic_year.year_id IN (:acYear)",nativeQuery=true)
	List<FacAgnstSanctnPostOthrState> getAllFacultyAgnstSanctionPostOthrState(@Param("instId")int instId,@Param("acYear") List<Integer> acYear);
